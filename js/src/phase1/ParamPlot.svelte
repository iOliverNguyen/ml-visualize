<script lang="ts">
  ...
  let params = $derived(snapshots.map(s => s.w))
  let minW = $derived(Math.min(...params))
  let maxW = $derived(Math.max(...params))
  let wRange = $derived(maxW - minW || 1)
  ...
  function scaleY(w: number): number {
    return height - padding.bottom - ((w - minW) / wRange) * plotHeight
  }

  // Update animated marker when currentStep changes
  $effect(() => {
    markerX.set(scaleX(currentStep))
    markerY.set(scaleY(snapshots[currentStep].w))
  })

  // Generate path data for the line
  let pathData = $derived(
    snapshots
      .map((s, i) => `${i === 0 ? 'M' : 'L'} ${scaleX(i)} ${scaleY(s.w)}`)
      .join(' ')
  )
</script>
...
<svg viewBox="0 0 {width} {height}" width="100%" height="auto" class="plot-svg">
  ...
  <text
    ...
    fill="#dc2626"
  >
    w: {snapshots[currentStep].w.toFixed(4)}
  </text>
</svg>
